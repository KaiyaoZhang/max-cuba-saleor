...
@patch("saleor.plugins.manager.PluginsManager.fulfillment_tracking_number_updated")
@patch(
...
def test_fulfillment_update_tracking_send_notification_true(
    ...
    mocked_fulfillment_tracking_number_updated_event.assert_called_once_with(
        fulfillment
    )
...
@patch("saleor.plugins.webhook.plugin.trigger_webhooks_async")
def test_fulfillment_tracking_number_updated_event_not_triggered(
    mocked_webhooks,
    permission_group_manage_orders,
    fulfillment,
    settings,
    ...
    # then
    mocked_webhooks.assert_not_called()
